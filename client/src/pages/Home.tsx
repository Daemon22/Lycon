...
const initialTabs: Tab[] = [  { id: 1, title: "Start", isPrivate: false, favicon: "/manus-storage/lycon-supplied-logo_58eb806e.png", history: [{ title: "Start", url: "lycon://start", kind: "local", view: "start", favicon: "/manus-storage/lycon-supplied-logo_58eb806e.png" }], historyIndex: 0 }];
...
function createDestination(value: string): PageRecord {
  ...
  if (localRoutes[normalized]) return { title: normalized, url: `lycon://${normalized}`, kind: "local", view: localRoutes[normalized], favicon: "/manus-storage/lycon-supplied-logo_58eb806e.png" };
  ...
  return { title: `Search: ${trimmed}`, url: `lycon://search?q=${encodeURIComponent(trimmed)}`, kind: "search", view: "search", query: trimmed, favicon: "/manus-storage/lycon-supplied-logo_58eb806e.png" };
}
...
export default function Home() {
  ...
  const syncQuery = trpc.library.get.useQuery(undefined, { enabled: isAuthenticated, retry: false });
  const accountExportQuery = trpc.library.export.useQuery(undefined, { enabled: isAuthenticated, retry: false });
  const syncPut = trpc.library.put.useMutation();
  const syncRemove = trpc.library.remove.useMutation();
  ...
  const newPrivateTab = () => {
    const id = Date.now();
    const startPage: PageRecord = { title: "Start", url: "lycon://start", kind: "local", view: "start", favicon: "/manus-storage/lycon-supplied-logo_58eb806e.png" };
  ...
  const exportAccountSnapshot = async () => {
    if (!isAuthenticated) { showToast("Sign in to export your account snapshot"); return; }
    try {
      const result = await accountExportQuery.refetch();
      const payload = result.data?.payload;
      if (!payload) { showToast("No account snapshot is saved yet"); return; }
      const blob = new Blob([payload], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `lycon-account-snapshot-${new Date().toISOString().slice(0, 10)}.json`;
      link.click();
      URL.revokeObjectURL(url);
      showToast("Account snapshot exported");
    } catch { showToast("Account snapshot could not be exported"); }
  };
  const requestDeleteAccountSnapshot = () => {
    if (!isAuthenticated) { showToast("Sign in to manage your account snapshot"); return; }
    setConfirmAction({ title: "Delete account snapshot?", copy: "This permanently removes the saved Lycon snapshot from your account. Your current local browser data will remain on this device.", confirmLabel: "Delete account snapshot", onConfirm: () => {
      setSyncStatus("Deleting account snapshot…");
      syncRemove.mutate(undefined, {
        onSuccess: () => { setSyncRevision(0); lastSyncedPayloadRef.current = ""; syncHydratedRef.current = false; trpcUtils.library.get.invalidate(); setSyncStatus("Account snapshot deleted"); showToast("Account snapshot deleted"); },
        onError: () => { setSyncStatus("Account snapshot could not be deleted"); showToast("Account snapshot could not be deleted"); },
      });
    } });
  };
...
function SettingsView({ settings, section, setSection, updateSetting, tabs, voskPack, onInstallVosk, onRemoveVosk, onExport, onImport, onExportAccount, onDeleteAccount, isAuthenticated, syncStatus, onSignIn }: { settings: SettingsState; section: SettingsSection; setSection: (section: SettingsSection) => void; updateSetting: <K extends keyof SettingsState>(key: K, value: SettingsState[K]) => void; tabs: Tab[]; voskPack: VoskPackState; onInstallVosk: () => void; onRemoveVosk: () => void; onExport: () => void; onImport: () => void; onExportAccount: () => void; onDeleteAccount: () => void; isAuthenticated: boolean; syncStatus: string; onSignIn: () => void }) {
  const voiceMode = settings.voiceMode ?? "online";
