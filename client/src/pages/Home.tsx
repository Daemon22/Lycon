...
import { syncNativeShields, listAgents, registerAgent as nativeRegisterAgent, unregisterAgent as nativeUnregisterAgent, type Agent } from "@/core/platformCapabilities";
import { Id, type TabId } from "../../../core/types/00_ids";
...
type SettingsSection = "appearance" | "privacy" | "search" | "permissions" | "voice" | "tabs" | "extensions" | "passwords" | "translate" | "tools" | "help";
type VoiceMode = "online" | "on-device" | "vosk";
...
export default function Home() {
  ...
  const { state: coreState, core: lyconCore, dispatch, navigate, goBack, goForward, createTab, activateTab, closeTab: closeCoreTab, setShields, bindEngineSurface, reload, registerAgent, unregisterAgent, agentSuggestion } = useLyconCore();
      ...
      <main className="lycon-main">
        ...
        <div className="toolbar tablet-compact-toolbar">
          ...
          <form className="address-wrap" onSubmit={submitAddress}>
            ...
            <VoiceInputButton onTranscript={setAddress} onStatus={showToast} processLocally={voiceMode !== "online"} voskReady={voskPack.status === "ready"} language={settings.voiceLanguage ?? "en-ZA"} />
          </form>
...
function CachedFavicon({ src, className }: { src: string; className: string }) {
  const [resolvedSrc, setResolvedSrc] = useState(src);
  useEffect(() => {
    let cancelled = false;
    setResolvedSrc(src);
    if (!src.startsWith("http")) return;
    const key = `lycon-favicon:${src}`;
    const cached = readStorage<string | null>(key, null);
    if (cached) { setResolvedSrc(cached); return; }
    fetch(src, { mode: "cors" }).then((response) => response.ok ? response.blob() : Promise.reject(new Error("favicon unavailable"))).then((blob) => new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(reader.error); reader.readAsDataURL(blob); })).then((dataUrl) => { if (cancelled) return; try { window.localStorage.setItem(key, dataUrl); } catch { /* cache is best effort */ } setResolvedSrc(dataUrl); }).catch(() => { if (!cancelled) setResolvedSrc(src); });
    return () => { cancelled = true; };
  }, [src]);
  return <img className={className} src={resolvedSrc} alt="" onError={(event) => { event.currentTarget.style.visibility = "hidden"; }} />;
}
...
function VoiceInputButton({ onTranscript, onStatus, processLocally, voskReady, language }: { onTranscript: (value: string) => void; onStatus: (message: string) => void; processLocally: boolean; voskReady: boolean; language: string }) {
  ...
  const startListening = async () => {
    ...
    recognition.onerror = (event: any) => { if (event.error === "language-not-supported" && !fallbackAttempted && fallbackLanguage) { fallbackAttempted = true; recognition.lang = fallbackLanguage; onStatus(`${preferredLanguage} is unavailable here; trying ${fallbackLanguage}`); recognition.start(); return; } setIsListening(false); onStatus(event.error === "not-allowed" ? "Microphone access was not allowed" : event.error === "language-not-supported" ? `${preferredLanguage} is not available in this browser` : "Voice input could not hear you"); };
...
function SettingsView({ settings, section, setSection, updateSetting, tabs, voskPack, onInstallVosk, onRemoveVosk, onExport, onImport, isAuthenticated, syncStatus, onSignIn, onShieldsChange, registerAgent, unregisterAgent, agentSuggestion, onToast, onConfirm }: { settings: SettingsState; section: SettingsSection; registerAgent: (agentName: string, domain: string) => Promise<CoreEvent[]>; unregisterAgent: (agentName: string) => Promise<CoreEvent[]>; agentSuggestion: (agentName: string, suggestedAction: Command) => Promise<CoreEvent[]>; onToast: (message: string) => void; onConfirm: (action: ConfirmAction) => void; setSection: (section: SettingsSection) => void; updateSetting: <K extends keyof SettingsState>(key: K, value: SettingsState[K]) => void; tabs: Tab[]; voskPack: VoskPackState; onInstallVosk: () => void; onRemoveVosk: () => void; onExport: () => void; onImport: () => void; isAuthenticated: boolean; syncStatus: string; onSignIn: () => void; onShieldsChange: (checked: boolean) => void }) {
  const voiceMode = settings.voiceMode ?? "online";
...
// Local agent-management surface for the Extensions settings panel. The Core is
// authoritative for the constitutional participant registry
// (RegisterAgent/UnregisterAgent/AgentSuggestion); native connector storage is
// handled at the platform boundary (listAgents / nativeRegisterAgent /
// nativeUnregisterAgent). The list is refreshed after every mutation because
// Core agent events are not projected into CoreState by handleCoreEvent.
function AgentPanel({ registerAgent, unregisterAgent, agentSuggestion, onToast, onConfirm }: {
  ...
  const loadAgents = () => {
    void listAgents()
      .then(setAgents)
      .catch(() => onToast("Could not load agents from the native boundary"));
  };
  ...
  const registerSubmit = async (event: FormEvent) => {
    event.preventDefault();
    if (!name.trim() || !endpoint.trim()) { onToast("Agent name and endpoint are required"); return; }
    setSaving(true);
    try {
      const events = await registerAgent(name.trim(), "browser");
      if (!events.some((e) => e.type === "AgentRegistered")) { onToast("Agent could not be registered with the Core"); return; }
      await nativeRegisterAgent({ name: name.trim(), endpoint: endpoint.trim(), model: model.trim() || undefined, apiKey: apiKey.trim() || undefined });
      onToast("Agent registered with the Core and native shell");
    } catch {
      onToast("Agent registration failed at the native boundary");
    } finally {
      setSaving(false);
      setName("");
      setEndpoint("");
  ...
  const requestRemove = (agent: Agent) => {
    onConfirm({
      title: "Remove agent?",
      copy: `Remove “${agent.name}” from this native shell and the Core?`,
      confirmLabel: "Remove agent",
      onConfirm: async () => {
        try {
          await unregisterAgent(agent.name);
          await nativeUnregisterAgent(agent.id);
          onToast("Agent removed from the Core and native shell");
        } catch {
          onToast("Agent removal failed at the native boundary");
        }
        loadAgents();
      },
    });
  ...
  const suggestAction = async (agent: Agent) => {
    const events = await agentSuggestion(agent.name, {
      commandId: Id.command(),
      participantId: agent.name,
      timestamp: Date.now(),
      type: "Navigate",
      tabId: "" as TabId,
      url: agent.endpoint,
    });
    if (events.some((e) => e.type === "CommandRejected")) {
      onToast("Agent suggestion recorded for surface re-issue (Navigate)");
    }
  };

  return (
    <div className="agents-panel">
      <SettingHeader title="Register an agent" copy="Agents run at the native boundary; Core registration authorizes the browser domain." />
      <form className="agent-form" onSubmit={registerSubmit}>
        <label className="setting-control"><span>Agent name</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. research" /></label>
        <label className="setting-control"><span>Endpoint</span><input type="url" value={endpoint} onChange={(event) => setEndpoint(event.target.value)} placeholder="https://api.example.com" /></label>
        <label className="setting-control"><span>Model</span><input value={model} onChange={(event) => setModel(event.target.value)} placeholder="e.g. gpt-4o" /></label>
        <label className="setting-control"><span>API key</span><input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="Stored by the native shell only" /></label>
        <button className="primary-btn" type="submit" disabled={saving}>{saving ? "Registering…" : "Register agent"}</button>
      </form>
      {agents.length === 0 ? (
        <div className="setting-note"><Puzzle size={16} /><div><strong>No agents configured</strong><p>Registered agents persist their connectors in the native shell.</p></div></div>
      ) : (
        <div className="list-panel">{agents.map((agent) => (
          <div className="list-row" key={agent.id}>
            <div className="row-icon"><Puzzle size={15} /></div>
            <div className="row-main"><strong>{agent.name}</strong><span>{agent.endpoint} · {agent.model || agent.protocol}</span></div>
            <button className="icon-btn" onClick={() => suggestAction(agent)} aria-label={`Suggest with ${agent.name}`} title="Suggest action"><Puzzle size={14} /></button>
            <button className="icon-btn" onClick={() => requestRemove(agent)} aria-label={`Remove ${agent.name}`} title="Remove agent"><Trash2 size={14} /></button>
          </div>
        ))}</div>
      )}
    </div>
